...
@lru_cache(maxsize=1)
def load_psychometric_floor_map():
    """
    Build lookup:
    pair_id (e.g. '011.png|021.png') -> floor_int
    """
    df = pd.read_csv("psychometric_results.csv")

    floor_map = {}
    for _, row in df.iterrows():
        pair_id = row["pair_id"]
        floor_map[pair_id] = int(row["floor_int"])
...
#             print(f"DEBUG: Folder {f} has left_indices: {left_indices}")
#             # choose up to 2 per folder
#             left_indices = left_indices[:MAX_REFS_PER_FOLDER]
#             for L in left_indices:
#                 floor_map = load_psychometric_floor_map()
#                 R_base = min(99, L + 10)
#                 pair_id = f"{L:03d}.png|{R_base:03d}.png"
                
#                 delta = floor_map.get(pair_id)
#                 R = min(99, L + 10 + delta)
...
#                     print(f"DEBUG: Missing Right Image: {right_rel}")
...
#     app.logger.info(
#         f"[PAIRWISE-FIRST Δ={delta + 10}] built {len(final_order)} pairs "
...
def build_pairwise_first_order(sid: str):
    ...
    trials_by_id = defaultdict(list)
    
    # 提前加载 floor_map，避免在循环内重复调用 lru_cache
    try:
        floor_map = load_psychometric_floor_map()
    except Exception as e:
        app.logger.error(f"Failed to load floor map: {e}")
        floor_map = {}

    last_delta_used = 10 # 给 logger 用的默认值
    ...
    for ident, f_list in folders_by_id.items():
        for f in f_list:
            ...
            for L in left_indices:
                ...
                # 修复：处理 delta 为 None 的情况
                delta = floor_map.get(pair_id, 0) 
                last_delta_used = delta
                R = min(99, L + 10 + delta)
                
                # 统一路径处理：去掉开头的 images/ 假设 _file_exists 会处理
                # 注意：这里确保路径与你的静态文件结构一致
                ref_dir_rel = f"images/{f}/ref_face"
                left_url, left_name = None, None

                # 尝试查找左图
                if _file_exists(f"{ref_dir_rel}/{L}.png"):
                    left_url = url_for("static", filename=f"{ref_dir_rel}/{L}.png")
                    left_name = f"{L}.png"
                elif _file_exists(f"{ref_dir_rel}/{L:03d}.png"):
                    left_url = url_for("static", filename=f"{ref_dir_rel}/{L:03d}.png")
                    left_name = f"{L:03d}.png"
                else:
                    left_url, left_name = resolve_ref_face_url(f, L)
                ...
                if not _file_exists(right_rel):
                    ...
                    if R > 0 and _file_exists(f"images/{f}/{R-1:03d}.png"):
                        ...
                        app.logger.warning(f"Skipping trial: Right image not found: {right_rel}")
                        continue
    ...
    app.logger.info(
        f"[PAIRWISE-FIRST] built {len(final_order)} pairs "
        f"(ids_order={identities_order})"
    )
    return final_order
